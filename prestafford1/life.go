...
const (
	...
	// Lets store whether the cell's next state in the fifth bit (32). Skipping the syntactic sugar this time.
	nextbit = 1 << 5
)
...
// nextState returns the next state of the cell.
...
// vivifyNext sets the cell's next state to alive.
func (c cell) vivifyNext() cell {
	return c | nextbit
}
...
// killNext sets the cell's next state to dead.
func (c cell) killNext() cell {
	return c &^ nextbit
}
	...
	for _, change := range m.changes {
		...
		if c.state() {
			if c.neighbors() != 0x2 && c.neighbors() != 0x3 {
				...
				m.field[change] = c.killNext()
			}
		} else if c.neighbors() == 0x3 {
			currentChanges = append(currentChanges, change)
			m.field[change] = c.vivifyNext()
		}
	}
	m.changes = []int{}
	for _, change := range currentChanges {
		if m.field[change].next() {
			m.makeAlive(change)
		} else {
			m.makeDead(change)
		}
